...
As this step involves exactly the same command discussed in [Episode 2](https://github.com/PawseySC/bio-workshop-18/blob/master/2.fastqc.md), the script `1.fastqc.sh` has already be completed, and is ready to be run. Let us have a look at its contents:
...
In this workshop, we are assuming that `gnuplot` is installed as a system package in the host machine, and therefore running it without containers.
...
How can we open it? By using a Docker flag we have seen in [Episode 1](https://github.com/PawseySC/bio-workshop-18/blob/master/1.containers.md)) when talking about interactive sessions, namely "`-i`":
