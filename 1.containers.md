...
- [`quay.io`](https://quay.io) : contains a large number of bioinformatic repositories, mostly from the [Biocontainers](https://github.com/BioContainers/containers) project.
...
Also note that by default Docker does not remove the container after running; as we don't need it after execution is complete, we are asking to remove it with `--rm`.
